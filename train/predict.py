...
class Detector:
    def __init__(self, model_path, model='ssd300', ctx='gpu', threshold=0.5, device_id=1, validation_threshold=0.5, batch_size=4, num_workers=2, nms_threshold=0.5):
        ...
        if model.lower() == 'ssd300_vgg16_voc':
            ...
            self.dataset= 'voc' # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< MUDAR ISSO DEPOIS
            self.width, self.height = 300, 300
...
def main():
    ...
    det = Detector(params, model='ssd300_vgg16_coco', ctx='gpu', threshold=0.1, device_id=1, batch_size=4, num_workers=2, nms_threshold=0.5)
